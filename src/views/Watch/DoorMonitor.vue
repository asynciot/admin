...
// 					if((_this.count+8) <= buffer.length){
						...
						_this.show.closeToOut = (buffer[_this.count+0]&0x80)>>7					//获取关到位输出信号
						_this.show.openTo =	(buffer[_this.count+0]&0x04)>>2								//获取开到位输入信号
						_this.show.closeTo = (buffer[_this.count+0]&0x08)>>3								//获取关到位输入信号				
						_this.show.door	= buffer[_this.count+1]&0x01								//正在开门信号
						_this.show.open	= (buffer[_this.count+1]&0x02)>>1								//正在开门信号
						_this.show.close =	(buffer[_this.count+1]&0x04)>>2						//正在关门信号
						_this.show.openKeep	= (buffer[_this.count+1]&0x08)>>3						//开门到位维持信号
						_this.show.closeKeep	= (buffer[_this.count+1]&0x10)>>4						//关门到位维持信号
						_this.show.stop	= (buffer[_this.count+1]&0x20)>>5								//停止输出信号
						_this.show.inHigh = (buffer[_this.count+1]&0x40)>>6							//输入电压过高
						_this.show.inLow = (buffer[_this.count+1]&0x80)>>7									//输入电压过低
						_this.show.outHigh = buffer[_this.count+2]&&0x01						//输出过流
						_this.show.motorHigh = (buffer[_this.count+2]&0x02)>>1				//电机过载
						_this.show.flySafe = (buffer[_this.count+2]&0x04)>>2						//飞车保护
						_this.show.closeStop = (buffer[_this.count+2]&0x08)>>3					//开关门受阻
						_this.show.position	= ((buffer[_this.count+2]&0xf0)<<4)+(buffer[_this.count+3]&0xff)		//获取位置信号
