				...
				Form.status(:model="show",label-position="left",:label-width="75")
					Row(:gutter="16")
						...
						Col(span="6")
							...
							Modal(v-model="modal1" title="警告"  @on-ok="warning")
								p()|你确定要终止此次实时数据么？
			Row.mb-10(:gutter="8")
				Col.padding(span="8")
					Card.card
	...
	export default {
		...
		methods: {
			initWebsocket(){ //初始化weosocket 　　　　　　
				this.websock = new WebSocket('ws://lengxia.s1.natapp.cc/devices/Monitor/socket?deviceId=45');
			...
			async websocketonopen() {
				...
				let res = await this.$api.monitor(this.query1);
			...
			websocketonmessage(e){ //数据接收 
				if(e.data=="closed"){
					...
					this.interval = redata.intervals
					this.getData(redata)
					console.log(redata)
				}
			},
			...
			getData(val) {
				...
				var _this = this
				_this.show.openIn = (buffer[0]&0x80)>>7
				_this.show.closeIn = (buffer[0]&0x40)>>6						//获取关门信号
				_this.show.openToOut = (buffer[0]&0x20)>>5					//获取开到位输出信号
				_this.show.closeToOut = (buffer[0]&0x10)>>4					//获取关到位输出信号
				_this.show.openTo =	(buffer[0]&0x02)>>1							//获取开到位输入信号
				_this.show.closeTo = (buffer[0]&0x01)								//获取关到位输入信号				
				_this.show.door	= (buffer[1]&0x80)>>7								//正在开门信号
				_this.show.open	= (buffer[1]&0x40)>>6								//正在开门信号
				_this.show.close =	(buffer[1]&0x20)>>5							//正在关门信号
				_this.show.openKeep	= (buffer[1]&0x10)>>4						//开门到位维持信号
				_this.show.closeKeep	= (buffer[1]&0x08)>>3					//关门到位维持信号
				_this.show.stop	= (buffer[1]&0x04)>>2								//停止输出信号
				_this.show.inHigh = (buffer[1]&0x02)>>1							//输入电压过高
				_this.show.inLow = (buffer[1]&0x01)									//输入电压过低
				_this.show.outHigh = (buffer[2]&0x80)>>7						//输出过流
				_this.show.motorHigh = (buffer[2]&0x40)>>6					//电机过载
				_this.show.flySafe = (buffer[2]&0x20)>>5						//飞车保护
				_this.show.closeStop = (buffer[2]&0x10)>>4					//开关门受阻
				_this.show.position	= ((buffer[2]&0x0f)<<8)+(buffer[3]&0xff)		//获取位置信号
				_this.show.current = (((buffer[4]&0xff)<<8)+(buffer[5]&0xff))/1000		//获取电流信号
				_this.show.speed = (((buffer[6]&0xff)<<8)+(buffer[7]&0xff))/1000
				if(_this.show.speed>32.767){
					_this.show.speed = _this.show.speed-65.535
				}
				_this.t_start = val.time
				_this.t_end = _this.t_start+val.nums*val.intervals
				_this.getX()
			},
			drawLine(){
