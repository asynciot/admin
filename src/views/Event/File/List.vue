...
//              },
       ...
       methods: {
         ...
         async getList() {
           this.loading = true
					 if (this.show.register=="all") {this.options.register=""}
           ...
           this.loading = false
		   if (fol.data.code ==0) {this.follow=fol.data.data.list}
           if (res.data.code === 0) {
         ...
         async toburn(item) {
			...
			item.register = "注册中"
           let res = await this.$api.regdevices({id: item.id,IMSI: item.device_IMSI, IMEI: item.IMEI,op:"register"})
           if (res.data.code === 0) {
             this.$Notice.success({
               title: '成功',
         ...
         async clear(item) {
           ...
           if (res.data.code === 0) {
			...
			async addfl(val){
				let res = await this.$api.addfollow({imei:val})
				if (res.data.code == 0) {
					this.$Notice.success({
						title: '成功',
						desc: '关注成功'
			...
			async delfl(val){
				let res = await this.$api.delfollow({device_id:val})
				if (res.data.code == 0){
					this.$Notice.success({
						title: '成功',
						desc: '不再关注该设备'
