							...
							Form(ref="form",:model="form",:rules="rules",:label-width="120")
								Row(:gutter="5")
									Col(span="20",offset="2")
										...
										Form-item(label="故障原因" v-if="form.type != '故障'")
											Col(span=12)
												checkbox(disabled v-model='fault[0]')|输入电压过低
											Col(span=12)
												checkbox(disabled v-model='fault[1]')|输入电压过高
											Col(span=12)
												checkbox(disabled v-model='fault[4]')|开关门受阻
											Col(span=12)
												checkbox(disabled v-model='fault[5]')|飞车保护
											Col(span=12)
												checkbox(disabled v-model='fault[6]')|电机过载
											Col(span=12)
												checkbox(disabled v-model='fault[7]')|输出过流
										Form-item(label="故障原因" v-if="form.type == '故障'")
											Col(span=12)
	...
	export default{	
		...
		methods:{
			...
			async sentalert(){
				var type=''
				var device_type=''
				var fault=this.fault[7]*128+this.fault[6]*64+this.fault[5]*32+this.fault[4]*16+this.fault[1]*2+this.fault[0]
